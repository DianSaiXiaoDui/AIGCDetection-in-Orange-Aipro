...
class SAFE_Ascend:
    def __init__(self, model_dir= "./SAFE_om_models"):
    ...
    def _load_models(self):
        ...
        # SAFE 模型
        safe_model_path = os.path.join(self.model_dir, "safe_body.om")
        safe_size = os.path.getsize(safe_model_path) / 1024 / 1024
        self.safe_model = AclLiteModel(safe_model_path)
        #print(f"✅ 已加载 safe_body.om, {safe_size:.2f} MB")
    ...
    def _preprocess_dwt(self, x):
        """
        DWT 预处理：提取 HH 频带并上采样回 256x256
        x: [1, 3, 256, 256] Tensor
        """
        Yl, Yh = self.dwt(x)
        hh_band = Yh[0][:, :, 2, :, :]  # 提取 HH 频带 [1, 3, 128, 128]
        resized_hh = F.interpolate(hh_band, size=(256, 256), mode='bilinear', align_corners=False)
        return resized_hh
    ...
    def preprocess_image(self, image_path):
        ...
        #print(f"✅ 预处理完成 - SAFE输入: {safe_input.shape}")
        return safe_input

    def body_inference(self, safe_input):
 
        # 推理分类器
        safe_input_batch = np.expand_dims(safe_input, axis=0)  # [1, 3, 256, 256]
        logits = self.safe_model.execute([safe_input_batch])
    ...
    def predict(self, image_path):
        ...
        try:
            ...
            # 1. 预处理
            safe_input = self.preprocess_image(image_path)
            ...
            # 3. 分类
            #print("🔍 分类推理...")
            logits = self.body_inference(safe_input)
...
def check_environment():
    ...
    # 检查模型目录
    model_dir = PROJECT_ROOT / "SAFE_om_models"
...
def SAFE_Init():
    detector = SAFE_Ascend(model_dir = PROJECT_ROOT / "SAFE_om_models")
    return detector
    
def SAFE_DeInit(detector = None):
    del detector
    print("🧹 资源清理完成")
       
# 项目接口一:SAFE对输入图像进行推理, 输出图像真假类别和置信
...
# 项目接口二:SAFE对输入视频抽取若干帧进行推理, 综合几帧推理结果，返回视频检测真假结果和平均置信度
